...
An Enterprise Data Model (EDM) was a single, organisation-wide logical or conceptual schema that attempted to describe every significant entity, attribute, and relationship across the business. Think of it as one canonical ER diagram to rule them all — a Platonic ideal of how the company's data *should* look, independent of any particular application or database.
...
## 2. Origins: The Golden Age of Centralised Data Architecture (1980s–2000s)
...
## 3. The Cracks: Why Enterprise Data Models Started Failing (2005–2015)

As enterprises grew in complexity, the EDM's weaknesses became systemic.

**The model could never keep up.** Business acquisitions, new SaaS tools, IoT data, and unstructured content generated schema changes faster than any central team could absorb. The EDM was always six months behind reality — or simply abandoned.
...
## 4. The Data Mesh Challenge (2019–Present)

Zhamak Dehghani's 2019 articulation of the **Data Mesh** was arguably the most direct intellectual challenge to the enterprise data model. Published on Martin Fowler's site, the original essay argued that centralised, monolithic data platforms — and by extension the canonical models that fed them — fail at scale because they concentrate domain knowledge in teams that don't have it.

The data mesh proposes four principles that collectively invert the EDM's assumptions:

**Domain-oriented data ownership** — the teams who produce data own and model it, rather than delegating to a central architecture team. **Data as a product** — data should be treated with the same product-thinking discipline as software: discoverable, understandable, trustworthy, and independently valuable. **Self-serve data platform** — a platform layer abstracts infrastructure complexity so domain teams can publish data products without being infrastructure specialists. **Federated computational governance** — standards and policies are agreed globally but enforced computationally, not through committee-reviewed data models.
...
The heavy modelling work happens at the domain level. In a medallion architecture (Bronze → Silver → Gold), domain models typically crystallise at the Silver and Gold layers, where source-aligned data is conformed to domain semantics.

A practical principle emerging in enterprise data platform work: **Bronze stays source-aligned; Silver is domain-aligned.** This means domain teams take responsibility for transforming raw ingested data into meaningful, governed, domain-specific structures — and those structures are the "real" models that matter.
...
A modern enterprise data platform (whether built on Databricks, Snowflake, Fabric, or a hybrid) does not need and should not wait for a monolithic enterprise data model. The platform's architecture — medallion layers, Unity Catalog governance, automated pipelines, domain ownership — provides the structural backbone that the EDM was supposed to deliver but rarely did.
...
The shift is from a single prescriptive artefact to an ecosystem of complementary capabilities — each more maintainable, more federated, and more closely coupled to the teams who actually use the data.
...
**Semantic layers and metrics stores** (dbt Semantic Layer, Databricks AI/BI) are emerging as the practical "shared understanding" layer. They define business metrics once and expose them consistently, which is arguably what the EDM's consumers actually wanted all along — not a model, but a consistent answer to "what does revenue mean?"

**AI-augmented data governance** is automating classification, tagging, and lineage — reducing the human effort that made the EDM unsustainable. LLMs can infer schema mappings and suggest domain alignments, accelerating what used to be months of data architect workshop time.

**Data products as the unit of architecture** are replacing both the monolithic data model and the monolithic data warehouse. Each data product carries its own schema, metadata, quality guarantees, and access policies — making it self-describing in a way the EDM always aspired to be.
...
**Composable, modular architectures** — API-first, event-driven, and platform-native — mean that integration is handled through interfaces and contracts rather than through upfront structural agreement.
...
- **Layer catalogs with domain-aligned schemas.** EDAP uses layer-based Unity Catalog catalogs (`prod_bronze`, `prod_silver`, `prod_gold`) with domain-aligned schemas within each (e.g. `prod_silver.asset_base`, `prod_gold.customer_bi`). Domain teams own and model the data within their schemas, consistent with the principle of pushing modelling responsibility to domain teams.
- **Domain BI and Exploratory schemas as the Gold layer.** Within the Gold catalog, domain-aligned schemas (e.g. `prod_gold.asset_bi`, `prod_gold.operations_exploratory`) contain certified, contracted data products. These are the governed, discoverable outputs that replace the EDM's aspiration for a single enterprise view. Each data product carries a data contract specifying schema, SLAs, quality expectations, ownership, and versioning.
- **Governed tags for classification and discovery.** Rather than encoding governance into a monolithic model, EDAP uses Unity Catalog governed tags (`waicp_classification`, `pi_contained`, `pi_type`, `access_model`, `soci_critical`, etc.) to classify, discover, and enforce access policies dynamically. Tags propagate through lineage and drive ABAC policies.
...
2. **Do invest in a thin enterprise ontology** — a shared set of business terms, domain boundaries, and critical entity definitions. Keep it conceptual and lightweight.

3. **Push modelling responsibility to domain teams.** They know their data best. Give them standards, patterns, and platform tooling — not a centralised schema they must comply with.
...
5. **Treat data quality and governance as continuous, automated capabilities** — not as properties of a model that must be manually maintained.
...
- Microsoft Azure (2024). "Data Domains — Cloud Adoption Framework." learn.microsoft.com.
...
|---|---|
| **Medallion Architecture** | Zone decomposition and data flow conventions that implement domain-aligned modelling in practice |
| **Data Domains** | Defines Water Corporation's seven data domains and conceptual data entities |
| **Data Governance Lifecycle** | Governance framework for data classification, quality, and stewardship |
| **EDAP Access Model** | Workspace topology, catalog bindings, and domain-based access control |
